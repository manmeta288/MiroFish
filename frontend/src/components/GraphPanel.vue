...
const wasSimulating = ref(false) // track whether we were simulating

// Dismiss simulation-end hint
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to idle, show end hint
...
const renderGraph = () => {
  ...
  // Process edge data; count edges between each node pair
  const edgePairCount = {}
  const selfLoopEdges = {} // self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges per node pair; collect self-loops
  tempEdges.forEach(e => {
  ...
  // Track processed edge count per node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge — one merged self-loop per node
    ...
    // Check edge direction vs normalised direction (src UUID < tgt UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: spread multiple edges; straight for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Flip curvature when edge direction is reversed
      // Ensures edges are distributed in the same reference frame without overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward centre to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw arc from node back to itself
      const loopRadius = 30
    ...
    // Perpendicular offset, proportional to distance, ensuring visible curve
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier: B(t)=(1-t)²P0+2(1-t)tP1+t²P2 at t=0.5
  ...
  // Link labels background (white, for legibility)
  ...
  // Save ref for external show/hide control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; don't restart simulation (click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect real drag (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only on first genuine drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Allow simulation to cool down only after a genuine drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (horizontal, no rotation)
