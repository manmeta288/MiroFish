    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation轮数配置 - 只有在配置生成完成且轮数计算出来后才显示 -->
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">若首次运行，强烈建议切换至‘Custom模式’减少Simulation轮数，以便快速预览效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles' || newStage === 'generating_profiles') {
    ...
    // 进入配置生成阶段，开始轮询配置
    if (!configTimer) {
      addLog('开始Generate Simulation Config...')
      startConfigPolling()
    }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // 用户Custom轮数，传递 max_rounds 参数
    params.maxRounds = customMaxRounds.value
    addLog(`Run Simulation，Custom轮数: ${customMaxRounds.value} rounds`)
  } else {
    // 用户选择保持自动生成的轮数，不传递 max_rounds 参数
    addLog(`Run Simulation，使用自动配置轮数: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 立即设置Expected Total（从prepare接口返回值获取）
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Sim Duration: ${data.summary.simulation_hours}小时`)
          addLog(`  ├─ Initial posts: ${data.summary.initial_posts_count}条`)
          addLog(`  ├─ Trending topics: ${data.summary.hot_topics_count}个`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: each round${tc.minutes_per_round} mins, total${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}轮`)
        }
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Sim Duration: ${res.data.summary.simulation_hours}小时`)
          addLog(`  └─ Initial posts: ${res.data.summary.initial_posts_count}条`)
        }
