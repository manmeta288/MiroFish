            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated but section may not be fully done) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract analysis question (CN + EN tool output)
    const queryMatch =
      text.match(/分析问题:\s*(.+?)(?:\n|$)/) ||
      text.match(/Analytical question:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    const reqMatch =
      text.match(/预测场景:\s*(.+?)(?:\n|$)/) ||
      text.match(/Simulation scenario:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    const factMatch =
      text.match(/相关预测事实:\s*(\d+)/) ||
      text.match(/(?:^|\n)-?\s*Related facts:\s*(\d+)/)
    const entityMatch =
      text.match(/涉及实体:\s*(\d+)/) ||
      text.match(/(?:^|\n)-?\s*Entities touched:\s*(\d+)/)
    const relMatch =
      text.match(/关系链:\s*(\d+)/) ||
      text.match(/(?:^|\n)-?\s*Relationship chains:\s*(\d+)/)
    ...
    const subQSection =
      text.match(/### 分析的子问题\n([\s\S]*?)(?=\n###|$)/) ||
      text.match(/### Sub-questions explored\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch =
          block.match(/相关事实:\s*(\d+)/) ||
          block.match(/Related facts:\s*(\d+)/)
        return {
    ...
    const relSection =
      text.match(/### 【关系链】\n([\s\S]*?)(?=\n###|$)/) ||
      text.match(/### Relationship chains\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const edgesMatch =
      ...
      text.match(/(?:^|\n)-?\s*Total edges:\s*(\d+)/)
    const activeMatch =
      text.match(/当前有效事实:\s*(\d+)/) ||
      text.match(/(?:^|\n)-?\s*Active facts:\s*(\d+)/)
    const histMatch =
      text.match(/历史\/过期事实:\s*(\d+)/) ||
      text.match(/(?:^|\n)-?\s*Historical \/ expired facts:\s*(\d+)/)
    ...
    const activeSection =
      ...
      text.match(/### Currently valid facts[^\n]*\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extract historical/expired facts (full, no limit)
    const histSection =
      text.match(/### 【历史\/过期事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/) ||
      text.match(/### Historical \/ expired facts[^\n]*\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Extract involved entities (full, no limit)
    const entitySection =
      text.match(/### 【涉及实体】\n([\s\S]*?)(?=\n###|$)/) ||
      text.match(/### Entities involved\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch =
      text.match(/\*\*采访人数:\*\*\s*(\d+)\s*\/\s*(\d+)/) ||
      text.match(/\*\*Agents interviewed:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract subject selection rationale
    const reasonMatch =
      text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/) ||
      text.match(/### Why these agents were selected\n([\s\S]*?)(?=\n---\n|\n### Interview transcript)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse each subject's selection rationale
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g. 1. **Agent_345 (index=1)**: Community member...
        ...
        // e.g. - Select Agent_601 (index 0): as community representative...
        if (!headerMatch) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Select\s+([^（(]+?)(?:\s*\(index\s*=?\s*\d+\))?\s*:\s*(.*)/i)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // e.g. - **Agent_601 (index 0)**: as community representative...
        if (!headerMatch) {
        ...
        if (name) {
          // Save previous subject's reason
          ...
          currentName &&
          line.trim() &&
          !line.match(/^未选|^综上|^最终选择|^Not selected|^In summary|^Final selection/i)
        ) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract heading (e.g. 'Student', 'Educator')
      ...
      const answerMatch =
        block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|\*\*Key quotes:|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        const twitterMatch =
          answerText.match(/【Twitter平台回答】\n?([\s\S]*?)(?=【Reddit平台回答】|$)/) ||
          answerText.match(/\[Twitter\]\n?([\s\S]*?)(?=\[Reddit\]|$)/)
        const redditMatch =
          answerText.match(/【Reddit平台回答】\n?([\s\S]*?)$/) ||
          answerText.match(/\[Reddit\]\n?([\s\S]*?)$/)
        ...
        // Platform fallback (backwards compat: single platform mark)
        if (!twitterMatch && redditMatch) {
          ...
          // No per-platform marks (very old format) — use full text
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (multi-format compat)
      const quotesMatch =
        block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/) ||
        block.match(/\*\*Key quotes:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" or > \u201Ctext\u201D
        if (!quoteMatches) {
    ...
    const summaryMatch =
      text.match(/### 采访摘要与核心观点\n([\s\S]*?)$/) ||
      text.match(/### Summary and takeaways\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch =
      text.match(/找到\s*(\d+)\s*条/) ||
      text.match(/Found\s+(\d+)\s+related item/i)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    const factsSection =
      text.match(/### 相关事实:\n([\s\S]*)$/) ||
      text.match(/### Related facts\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Edge Chains (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Edge Chains'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift-query generated sub-problems'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No entities yet'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No edge chains yet')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Independent platform selection state per Q&A pair
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fall back to numeric format if not matched
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Return full text if no numbering or single question
      if (matches.length <= 1) {
    ...
    // Check if a question has genuine dual-platform answers
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real, different answers
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only with real dual answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (shown in outer wrapper already)
  ...
  // Handle line breaks: blank lines → paragraphs, newlines → <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <br> before paragraph after block element
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic in nextTick after loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If done, scroll top; else scroll bottom to follow latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Find content after 'Final Answer:' (multi-format)
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* dark grey, unchanged with state */
  font-weight: 500;
}
