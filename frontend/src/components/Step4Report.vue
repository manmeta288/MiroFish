...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics — match 'Related prediction facts: N' format
    const factMatch = text.match(/相关预测事实:\s*(\d+)/)
    const entityMatch = text.match(/涉及实体:\s*(\d+)/)
    const relMatch = text.match(/ edges链:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-questions (full, no limit)
    ...
    // Extract key facts (full, no limit)
    ...
    // Extract edge chains (full, no limit)
    const relSection = text.match(/### 【 edges链】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract active facts (full, no limit)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts (full, no limit)
