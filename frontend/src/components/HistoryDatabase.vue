<template>
  ...
  >
    <!-- Background decoration: tech grid (shown when projects exist) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card header: simulation_id and feature status -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 (Run Simulation) and Step 5 (Deep Interaction) must be started live and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Compute required height: rows*height + (rows-1)*gap + bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per row for centred layout
...
// Navigate to Map Construction page
const goToProject = () => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always track)
        ...
        // If target equals current, no action needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounced state switching to prevent flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state is still needed
          ...
          // Release lock after animation; check for pending changes
          setTimeout(() => {
            isAnimating = false
    ...
    },
    {
      ...
      // Shrink rootMargin at viewport bottom — requires more scrolling to expand
...
// Watch route; reload data when returning home
...
onMounted(async () => {
  // Ensure DOM rendered before loading data
  await nextTick()
  await loadHistory()
  
  // Initialise observer after DOM renders
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Reload data on activation when using keep-alive
onActivated(() => {
  loadHistory()
})
...
/* CSS background for fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchored top-left; expands only at bottom */
...
.cards-container {
  ...
  /* min-height computed dynamically by JS */
}
...
.card-progress.in-progress { color: #F59E0B; }  /* Active - orange */
