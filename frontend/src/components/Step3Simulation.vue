...
// Format elapsed simulation time (rounds × mins/round)
...
const doStartSimulation = async () => {
  ...
  // Reset state first to avoid interference from previous run
  resetAllState()
...
// Track last round per platform to detect changes
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is complete (via runner_status or platform status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: backend may not have updated runner_status yet
...
const checkPlatformsCompleted = (data) => {
  ...
  // If one platform done, check all enabled ones
  // Determine if platform is enabled via actions_count
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Fetch full action list via all_actions
      ...
      // No auto-scroll; let user browse the timeline
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Generate report request sent, please wait…')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ Report generation start failed: ${res.error || 'Unknown error'}`)
...
onMounted(() => {
  addLog('Step 3 simulation run initialising')
  if (props.simulationId) {
    doStartSimulation()
  }
})
