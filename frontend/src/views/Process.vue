      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Map Construction in progress</p>
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyses content and auto-generates an ontology schema (entity types + edge types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">Map Construction</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Description</div>
                <div class="detail-content">
                  Documents are chunked using the ontology and the knowledge graph is built, extracting entities and edges.
                </div>
              </div>
...
const currentPhase = ref(-1) // -1: uploading, 0: generating ontology, 1: map construction, 2: done
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Map Construction'
...
// Initialising — new or existing project
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = 'Project initialisation failed: ' + (err.message || 'Unknown error')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling in-progress build
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'Starting map construction…'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Map construction task started…'
      ...
      error.value = response.error || 'Failed to start map construction'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Failed to start map construction: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}
...
const startGraphPolling = () => {
  ...
  // Auto-fetch graph data every 10 s
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Map construction failed: ' + (task.error || 'Unknown error')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  // Draw edges (with wide transparent click target)
